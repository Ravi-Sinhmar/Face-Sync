    ...
    <div className=" font-rob flex flex-col h-svh w-full items-center justify-center gap-2 pb-9">
      ...
      <div className="flex flex-col  items-center gap-4">
        ...
        <div className="flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-gray-500 font-[400] text-center w-10/12">{!isClick ? "Connect, Laugh, and Create moments. Anytime, Anywhere with Love😚": isLink ? "Copy and Share the link with your friend :" :"Enter name, Generate link, Share Link with friend and roast him/her😎" }</p>
        </div>
      </div>
...
{isLink !== null ?  <div className="px-2 py-1 rounded-lg gap-2  flex flex-col items-center justify-center ">
      <h3 className="text-lg text-blm px-2 rounded-md font-[500]">Copy & Share</h3>
      <div className="flex items-center justify-center gap-2">
      <input className="bg-blm w-11/12 px-2 py-2 text-sm rounded-md ring-1 "
        ref={linkRef}
        type="text"
        value={isLink}
        readOnly
      ...
      />
      <button className="bg-blf  px-4 py-2 text-sm rounded-md text-blm" onClick={copyToClipboard}>Copy</button>
      </div>
